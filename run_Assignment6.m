...
print_2a(a, sigma, dt, N, tree, P_market_10y, P_model_10y, t_ex, x_node, swapValue, parRate, K, bermPrice, Notional_Ex2);
...
for s = 1:length(stepsList)
    dt_test = 1 / stepsList(s);
    N_test = round(T_HW / dt_test);
    tree_test = build_OU_tree(a, sigma, dt_test, N_test);
    prices(s) = price_bermudian_HW_tree(tree_test, curve, K, maturity, exerciseTimes, a, sigma);
end

euroPricesTree = zeros(length(exerciseTimes), 1);
for j = 1:length(exerciseTimes)
    euroPricesTree(j) = price_bermudian_HW_tree(tree, curve, K, maturity, exerciseTimes(j), a, sigma);
end

lowerBoundTree = max(euroPricesTree);
upperBoundTree = sum(euroPricesTree);

print_2b(stepsList, prices, exerciseTimes, euroPricesTree, lowerBoundTree, upperBoundTree, bermPrice, Notional_Ex2);
...
print_2c(exerciseTimes, europeanPricesJ, lowerBoundJ, upperBoundJ, bermPrice, Notional_Ex2);
